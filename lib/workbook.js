...
const girlsScoreData = require('../data/girls-score-data.json');
...
function loadSheets() {
  return scoreData.sheets.map((sheet) => {
    if (sheet.name !== 'י') {
      return sheet;
    }

    const headers = [...sheet.table.headers];
    const rows = sheet.table.rows.map((row) => [...row]);
